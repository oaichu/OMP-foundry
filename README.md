...
- **Bounded planning** — Plan3 remains three blocking stages rather than a second orchestration system. Each stage gets a fresh child context, performs one bounded evidence pass, and writes exactly one artifact. All three keep `thinking-level: max`; reliability comes from bounded evidence and fresh-context retry, not from weakening reasoning. Stage agents intentionally have no web search, helper spawning, or broad AST exploration. Foundry arms a hard watchdog of **6 minutes** per stage (override with `FOUNDRY_STAGE_TIMEOUT_MS`, clamped to 2–30 minutes); a stalled child is aborted, the stage remains unchanged, and the parent is instructed to respawn that same stage once with a fresh context. A completed artifact is the only thing that advances the state.
