...
tools: read, grep, glob, web_search, foundry_plan_write, ast_grep
...
READ PRODUCT, the draft, and relevant repository evidence. WRITE only `docs/planning/PLAN_REVIEW.md` using the injected capability with `foundry_plan_write`; native write/edit calls are denied. Do not rewrite the draft, do not write MASTER_PLAN, and never implement.
