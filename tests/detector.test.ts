...
describe("unified RepoFacts detector", () => {
	...
	test("nested workspace UI is not silently classified as backend-only", () => { const cwd = dir(); mkdirSync(join(cwd, "apps", "web"), { recursive: true }); writeFileSync(join(cwd, "apps", "web", "package.json"), "{\"dependencies\":{\"react\":\"latest\"}}\n"); expect(detectRepo(cwd).uiConfidence).toBe("unknown"); });
});
