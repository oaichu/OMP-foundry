...
describe("hard execution boundary", () => {
	test("eval is always denied", () => expect(denyToolCall("eval", { code: "1+1" }, defaultState())?.reason).toContain("APPROVAL_REQUIRED"));
	test("arbitrary bash and redirects are denied", () => { expect(denyToolCall("bash", { command: 'echo x > "docs/MASTER_PLAN.md"' }, locked())?.reason).toContain("APPROVAL_REQUIRED"); expect(denyToolCall("bash", { command: "python - <<'PY'\nprint(1)\nPY" }, locked())?.reason).toContain("APPROVAL_REQUIRED"); });
	test("agent release actions are always denied even when release is green", () => { const state = locked(); state.release.ready = true; expect(denyToolCall("bash", { command: "git push origin main" }, state)?.reason).toContain("APPROVAL_REQUIRED (release)"); });
