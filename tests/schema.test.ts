...
describe("state schema migration", () => {
	...
	test("review and manifest evidence roundtrip", () => {
		const state = defaultState(); state.aatp.manifest_sha256 = "manifest"; state.aatp.baseline_sha = "a".repeat(40); state.aatp.governed_commits = ["b".repeat(40)]; state.tickets["AATP-1"] = { id: "AATP-1", status: "completed", allowed_files: ["src"], forbidden_files: [], risk: "normal", review: "APPROVE", review_by: "reviewer", review_evidence_sha256: "review", implementation_evidence_sha256: "impl" };
		const again = parseState(serializeState(state)); expect(again.aatp.manifest_sha256).toBe("manifest"); expect(again.aatp.baseline_sha).toBe("a".repeat(40)); expect(again.aatp.governed_commits).toEqual(["b".repeat(40)]); expect(again.tickets["AATP-1"]?.review_by).toBe("reviewer"); expect(again.tickets["AATP-1"]?.review_evidence_sha256).toBe("review");
	});
